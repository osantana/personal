...
FFI_ROOTDIR="${HOMEBREW_HOME}/opt/libffi"
if [ -d "${FFI_ROOTDIR}" ]; then
    LDFLAGS="${LDFLAGS} -L${FFI_ROOTDIR}/lib"
    CPPFLAGS="${CPPFLAGS} -I${FFI_ROOTDIR}/include"
    PKG_CONFIG_PATH="${PKG_CONFIG_PATH+${PKG_CONFIG_PATH}:}${FFI_ROOTDIR}/lib/pkgconfig"
fi

BERKELEYDB_ROOTDIR="${HOMEBREW_HOME}/opt/berkeley-db"
if [ -d "${BERKELEYDB_ROOTDIR}" ]; then
    PATH="${BERKELEYDB_ROOTDIR}/bin:${PATH}"
    LDFLAGS="${LDFLAGS} -L${BERKELEYDB_ROOTDIR}/lib"
    CPPFLAGS="${CPPFLAGS} -I${BERKELEYDB_ROOTDIR}/include"
